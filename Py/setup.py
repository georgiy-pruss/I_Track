...
# The manifest will be inserted as resource into i_track.exe.  This
# gives the controls the Windows XP appearance (if run on XP ;-)
#
# Another option would be to store if in a file named
# i_track.exe.manifest, and probably copy it with the data_files
# option.
#
manifest_template = '''
...
i_track = dict( script = "i_track.py",
  other_resources = [(RT_MANIFEST, 1,
                      manifest_template % dict(prog="i_track"))],
  dest_base = r"i_track") # prog\...
...
options = {"py2exe": {"compressed": 1,
                      "optimize": 2,
                      "dll_excludes":
                        ["MSVCR71.dll","MSVCp71.dll","python25.dll","wxmsw28uh_vc.dll"]}}
                      #"excludes": excludes,
                      #"includes": includes,
...
class InnoScript:
  def __init__(self, name, lib_dir, dist_dir,
                windows_exe_files = [],
                lib_files = [],
                version = "1.0"):
  ...
  def create(self, pathname="dist\\i_track.iss"):
    ...
    #self.lib_files.append( "SortUtl.dll" )
    #self.lib_files.append( "PrintUtl.dll" )
    print >> ofi, r"[Files]"
  ...
  def compile(self):
    ...
    try:
      ...
      res = ctypes.windll.shell32.ShellExecuteA(0, "compile", self.pathname,
                                                None, None, 0)
      if res < 32:
...
#~ This would create a subdirectory dist\bitmaps, containing the two .gif files,
#~ and a subdirectory dist\fonts, containing all the .fnt files.

#~ setup(service=["MyService"]) -- as service
#~ setup(com_server=["win32com.server.interp"]) -- com service
